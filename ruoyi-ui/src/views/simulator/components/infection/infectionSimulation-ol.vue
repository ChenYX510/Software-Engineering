...
export default {
  ...
  data() {
    return {
      ...
      currentRiskSource: null,            // 当前风险点数据源
  ...
  methods: {
    ...
    async onStatusChange() {
      ...
      if (this.infectionModel == "gamma") {
      ...
      else if (this.infectionModel == "performance") {
        {
          ...
          let len = Math.max(this.data_performance.ModelData.length,this.data_performance.BaselineData.length);
          for (let i = 1; i <= len; ++i) {
    ...
    updateRiskPoint() {
        ...
        this.$emit("onChangeRisk",this.currentRiskSource);
      });
